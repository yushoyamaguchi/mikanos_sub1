...
SYSCALL(SocketRecvFrom) {
  struct socket *s = socketget((int)arg1);
  uint64_t ret = socketrecvfrom(s, (char *)arg2, (int)arg3, (struct sockaddr *)arg4, (int *)arg5);
  return {ret, 0};
}

SYSCALL(SocketSendTo) {
  struct socket *s = socketget((int)arg1);
  uint64_t ret = socketsendto(s, (char *)arg2, (int)arg3, (struct sockaddr *)arg4, (int)arg5);
  return {ret, 0};
}
...
SYSCALL(MutexLock) {
  uint64_t ret = mutex_lock((mutex_t *)arg1);
  return {ret, 0};
}

SYSCALL(MutexUnlock) {
  uint64_t ret = mutex_unlock((mutex_t *)arg1);
  return {ret, 0};
}
...
SYSCALL(GetSampleMutex) {
  uint64_t ret = (uint64_t)get_sample_mutex();
  return {ret, 0};
}
