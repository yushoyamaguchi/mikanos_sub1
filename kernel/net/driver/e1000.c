...
static unsigned int
e1000_reg_read(struct e1000 *adapter, uint16_t reg)
{
    return *(volatile uint32_t *)(adapter->mmio_base + reg);
}

static void
e1000_reg_write(struct e1000 *adapter, uint16_t reg, uint32_t val)
{
...
e1000_open(struct net_device *dev)
{
    ...
    // enable RX/TX
    e1000_reg_write(adapter, E1000_RCTL, e1000_reg_read(adapter, E1000_RCTL) | E1000_RCTL_EN);
    e1000_reg_write(adapter, E1000_TCTL, e1000_reg_read(adapter, E1000_TCTL) | E1000_TCTL_EN);
    // link up
    e1000_reg_write(adapter, E1000_CTL, e1000_reg_read(adapter, E1000_CTL) | E1000_CTL_SLU);
    return 0;
}
...
static void
e1000_rx_init(struct e1000 *adapter)
{
    ...
    // rx descriptor lengh
    e1000_reg_write(adapter, E1000_RDLEN, (uint32_t)(RX_RING_SIZE * sizeof(struct rx_desc)));
    // setup head/tail
...
static void
e1000_tx_init(struct e1000 *adapter)
{
    ...
    e1000_reg_write(adapter, E1000_TDLEN, (uint32_t)(TX_RING_SIZE * sizeof(struct tx_desc)));
    // setup head/tail
...
static void
e1000_receive(struct e1000 *adapter)
{
    ...
    while (1) {
        ...
        do {
            if (desc->length < 60) {
                errorf("%s: short packet (%d bytes)", adapter->dev->name, desc->length);
                break;
            }
            if (!(desc->status & E1000_RXD_STAT_EOP)) {
                errorf("%s: not EOP! this driver does not support packet that do not fit in one buffer", adapter->dev->name);
                break;
            }
            if (desc->errors) {
                errorf("%s: rx errors (0x%x)", adapter->dev->name, desc->errors);
                break;
            }
            debugf("%s: %u bytes data received", adapter->dev->name, desc->length);
...
static struct e1000 *
e1000_alloc(uintptr_t mmio_base)
{
    struct e1000 *adapter;
    uint32_t v1, v2;
    ...
    debugf("mmio_base = %0x08x, addr = %02x:%02x:%02x:%02x:%02x:%02x", adapter->mmio_base,
        adapter->addr[0], adapter->addr[1], adapter->addr[2], adapter->addr[3], adapter->addr[4], adapter->addr[5]);
    for (int n = 0; n < 128; n++) {
...
int
e1000_init(uintptr_t mmio_base)
{
    struct e1000 *adapter;
    struct net_device *dev;
